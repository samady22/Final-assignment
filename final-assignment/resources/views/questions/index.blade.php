...
<div class="container">
    <div class="row">
        <div class="col-md-8 border-right  border-success">
            <h5 class="h5">Select files to generate task</h5>
            <form id="fileSelectionForm" action="{{ route('getRandomQuestion') }}" method="POST">
                @csrf
                <div class="form-check">
                    @foreach ($latexFileNames as $latexFileName)
                    <input class="form-check-input" type="checkbox" name="latex_files[]" value="{{ $latexFileName }}" id="fileCheckbox{{ $loop->index }}">
                    <label class="form-check-label" for="fileCheckbox{{ $loop->index }}">{{ $latexFileName }}</label><br>
                    @endforeach
                </div>
                <button type="submit" class="btn btn-success mt-3">Generate task</button>
            </form>
        </div>
        <div class="col-md-4 ">
            <h6>Description:</h6>
    ...
    <div class="row mt-5">
        <div class="col-md-12">
            <h6 class="h6 text-center">Assignments of <span class="text-secondary rounded p-1">{{$user}}</span></h6>
            <hr>
            <div class="row ">
                @foreach ($assignments as $index => $assignment)
                <div class="card {{ $assignment->status == 1 ? 'bg-success' : 'bg-danger' }} m-2" style="width: 13rem;">
                    <div class="card-body text-center">
                        <h6 class="card-title text-light">Task {{ $index + 1 }}</h6>
                        <button type="button" class="btn btn-info " data-toggle="modal" data-target="#myModal{{ $assignment->id }}">
                            See task
                        </button>
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                                <div class="modal-content " style="background-color: #E6FDF8">
                                    <div class="modal-header">
                                        <h6 class="modal-title" id="myModalLabel">Assignment</h6>
                                    ...
                                    <div class="modal-body text-center">
                                        ...
                                        @endif
                                        <h6><span class="bg-warning text-dark rounded">Status: {{ $assignment->status }}</span></h6>
